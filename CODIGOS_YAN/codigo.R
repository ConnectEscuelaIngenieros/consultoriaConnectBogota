...
###### 2 LEER DATOS (MISMA LOGICA QUE MI CODIGO ORIGINAL)
  ...
  names(datos_completos) <- c(
    ...
    "Insumo_Presup",      # ...6 (15.5, 13.87) VALOR MONETARIO - USAR
    "Item_Presup",        # ...7
    "Capitulo_Presup",    # ...8
    "Cantidad_Proy",      # ...9 (321.10, 57.76) CANTIDAD, NO USAR
    "Insumo_Proy",        # ...10 (13.8, 13.87) VALOR MONETARIO - USAR
    "Item_Proy",          # ...11
    "Capitulo_Proy"       # ...12
  )
  ...
  # 3. CONFIGURAR IPC (MISMO QUE TU CODIGO)
...
###### 5 APLICAR LIMPIEZA Y CALCULOS (MISMA LOGICA)
  datos_final <- datos_completos
  
####### Limpiar las columnas CORRECTAS (Insumo, columnas 6 y 10)
...
####### 6 IDENTIFICAR CAPITULOS (MISMO CODIGO)
  ...
  addStyle(wb, "Presupuesto Completo VP", headerStyle, rows = 1, cols = 1:8)
